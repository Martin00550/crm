...
export function PolicyLedgerTable({ ledger, isDemo = false }: { ledger: Policy[], isDemo?: boolean }) {
  ...
  return (
    <div className="bg-white rounded-3xl border border-black/5 shadow-sm overflow-hidden font-body flex flex-col h-[700px]">
      <div className="px-8 py-6 border-b border-black/5 flex flex-col lg:flex-row justify-between items-center gap-6 shrink-0">
        ...
        <div className="flex items-center gap-3">
          {selectedPolicies.size > 0 && (
            <div className="flex gap-2 mr-4 border-r border-black/5 pr-4">
          ...
          <button onClick={() => exportPolicyLedgerToPDF(filteredLedger, 'ledger')} className="p-3 min-w-[44px] min-h-[44px] flex items-center justify-center text-on-surface/40 hover:text-secondary transition-colors"><FileText className="w-4 h-4" /></button>
        ...
        <table className="w-full text-left border-collapse table-fixed">
          <thead className="sticky top-0 z-10">
            <tr className="border-b border-black/5 bg-slate-50/90 backdrop-blur-sm">
              ...
              <th className="px-6 py-4 text-[11px] font-bold text-on-surface/30 uppercase tracking-widest cursor-pointer group" onClick={() => handleSort('clientName')}>
                <div className="flex items-center gap-2">Insured {getSortIcon('clientName')}</div>
              </th>
              <th className="px-6 py-4 text-[11px] font-bold text-on-surface/30 uppercase tracking-widest cursor-pointer group w-1/5" onClick={() => handleSort('carrier')}>
                <div className="flex items-center gap-2">Carrier {getSortIcon('carrier')}</div>
              </th>
              <th className="px-6 py-4 text-[11px] font-bold text-on-surface/30 uppercase tracking-widest w-1/5">Type</th>
              <th className="px-6 py-4 text-[11px] font-bold text-on-surface/30 uppercase tracking-widest text-right w-1/6" onClick={() => handleSort('premium')}>
                <div className="flex items-center justify-end gap-2">Premium {getSortIcon('premium')}</div>
              </th>
              <th className="px-8 py-4 text-[11px] font-bold text-on-surface/30 uppercase tracking-widest text-right w-1/6">Expiration</th>
            </tr>
          </thead>
            ...
            {rowVirtualizer.getVirtualItems().map((virtualItem) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-sm text-on-surface/60 font-medium truncate w-1/5">{policy.carrier}</td>
