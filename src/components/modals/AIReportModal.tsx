...
const DEMO_RESPONSES = [
  (policyType: string, carrier: string) => `FORENSIC AUDIT COMPLETE: Our analysis of the current "Hard Market" indicates that ${carrier} is adjusting ${policyType} premiums upwards by 12%. This is driven by aggregate coastal liability shifts. However, your loss-run history remains superior. STRATEGY: Maintain current placement but prepare a "Stability Brief" for the insured.`,
  (policyType: string, carrier: string) => `RATE ANALYSIS: The 15% spike in ${policyType} with ${carrier} is a result of carrier-wide re-insurance hikes. We have cross-referenced this against 8 competing carriers. PROPOSAL: Deployed a "Forensic Rate Explainer" to the insured to justify the adjustment based on re-insurance data.`,
  (policyType: string, carrier: string) => `PORTFOLIO ALERT: ${policyType} with ${carrier} is currently priced at a 14% discount to current market replacement cost. This is a "Preferred Placement" that must be secured. ACTION: Initiate 90-day automated outreach to confirm renewal before carrier re-rating occurs.`,
];
...
export function AIReportModal({ 
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Forensic AI Rate Analysis" maxWidth="lg">
      <div className="space-y-8 font-body p-2">
        ...
        {isGenerating ? (
          <div className="flex flex-col items-center justify-center py-16 bg-slate-50/30 rounded-[32px] border border-black/5">
            ...
            <p className="text-sm font-black text-on-surface uppercase tracking-[0.2em] animate-pulse">Syncing carrier intelligence...</p>
            <p className="text-[10px] font-bold text-on-surface/20 uppercase tracking-widest mt-2 italic">Scanning global reinsurance markets & loss-run patterns</p>
          </div>
        ) : report ? (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="p-8 bg-surface rounded-[32px] border border-black/5 shadow-xl relative overflow-hidden">
              ...
              <div className="flex items-center gap-3 mb-6 relative z-10">
                <span className="text-[10px] font-black text-secondary uppercase tracking-[0.3em]">Forensic Briefing</span>
            ...
            {isSent ? (
              <div className="flex items-center justify-center gap-4 p-6 bg-secondary/5 text-secondary rounded-[24px] border border-secondary/10 shadow-sm animate-in zoom-in-95">
                ...
                <span className="font-black text-sm uppercase tracking-[0.2em]">Forensic Explainer Dispatched</span>
              </div>
            ) : (
